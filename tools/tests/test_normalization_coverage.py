...
class ExhaustiveNormalizationCoverageTests(unittest.TestCase):
    def test_every_assigned_core_syriac_codepoint_is_classified(self):
        """Freeze the coverage boundary for the assigned U+0700–U+074F block."""
        ...
        assigned = {
            cp for cp in range(0x0700, 0x0750)
    ...
    def test_every_single_point_alias_on_every_carrier_class(self):
        above_expected = {
            BETH: QUSSHAYA,
            WAW: RWAHA,
            YODH: GENERIC_DOT_ABOVE,
            MIM: GENERIC_DOT_ABOVE,
        }
        below_expected = {
            BETH: RUKKAKHA,
            WAW: HBASA_ESASA_DOTTED,
            YODH: HBASA_ESASA_DOTTED,
            MIM: GENERIC_DOT_BELOW,
        }
    ...
    def test_every_east_vowel_and_special_mark_has_a_legal_case(self):
        cases = [
            ...
            MIM + "\u0739",  # zlama qashya
            WAW + "\u073f",  # rwaha
            WAW + "\u073c",  # u carrier vowel
            YODH + "\u073c",  # i carrier vowel
            MIM + SYAME,
            MIM + BREVE_BELOW,
            MIM + BETWEEN_ABOVE,
            MIM + BETWEEN_BELOW,
    ...
    def test_all_explicitly_out_of_scope_inputs_are_removed(self):
        debris = (
            set(SYRIAC_PUNCTUATION)
            | {SYRIAC_ABBREVIATION_MARK}
            | set(PRESENTATIONAL)
            | set(ACCENT_CANTILLATION)
            | set(LATIN_PUNCTUATION_SUBSTITUTES)
        )
        for char in debris:
    ...
    def test_mater_and_word_final_shapes_remain_literal_for_later_transliteration(self):
        # The normalizer/audit must not infer matres. It simply preserves the
        # exact letters; the later transliterator applies the positional rules.
