...
Settled here, not repeated in the Glossary. A text falling outside all four has its source of record designated explicitly **before entries are built**, and recorded with the confirmed text.
...
**Citing a section always names its file** — "Translit §10", "General Rules §10". Both carry a §10 and a §7. Withdrawn sections are retained as withdrawal notes, never renumbered.
...
4. Variance is recorded, not suppressed. Each rendering carries its own count. Keep repeated terms stable (§3), but do not flatten a genuine split.
...
## 11. Check Suite
Run after every glossary write, and after any edit to a confirmed text. Each has caught a real error. **Diagnose a flag before calling it a defect** — a new check is new code, and these have produced more false positives than the files have produced faults.
...
Dukhrana, CAL, and Payne Smith are blocked or image-only from a sandboxed environment, and reachable from a local build with unrestricted browsing. Check availability rather than assuming either way.
