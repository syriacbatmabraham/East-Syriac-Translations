...
- **Zero-byte placeholders may reserve future texts and their ordered filenames.** A zero-byte `.txt` or `.md` file is not a repository-confirmed text, receives no `sources/sources.yaml` entry or Glossary citations, and is ignored by confirmed-corpus validation. Once populated, it ceases to be a placeholder and must satisfy all confirmed-text, provenance, Glossary, and validation requirements before it is repository-confirmed.
...
- **Alternatives supplied by the source itself use square brackets in all three blocks.** Record each under `source_alternatives` in `sources/sources.yaml`, with its logical `line`, one-based `bracket` number on that line (excluding editorial labels), descriptive `label`, and exact bracketed canonical `reading`. These declarations identify alternatives in the file's source of record; they do not designate another witness. Each declared group must contain whole words and use non-nested brackets. Its Syriac, canonical transliteration, and English remain subject to ordinary validation and Glossary indexing. Other bracketed source readings retain their witness-apparatus treatment. English-only interpretive supplements remain excluded from lexical contexts.
...
3. Each occurrence records its English context and a citation. **A citation names the text and the line.** Standing alone: `(Creed Line 5)`, `(Teshbhotha l'Alaha Line 2)`. One file, one text, one stable label. The label is the `citation_label` recorded for that confirmed file in `sources/sources.yaml`. A unit belonging to an office carries the office in its label: `(Ferial Slotha d'Ramsha, Line 3)`. An office name alone is not a label — the unit is. Where a text's own numbering is standard, it is used instead. **A citation string, once written, is reused exactly**; a label is fixed by first use, changing only for a rendering change or correction.
   ...
   **An explicitly registered source alternative uses the text's own citation label and line**, without an external-witness qualifier. Its tokens are indexed normally; the same-locus witness-duplicate exemption does not apply merely because a word occurs both inside and outside this source alternative.
...
6. **A non-compositional phrase is tabled in its own section and governs its components.** It carries the same fields as a form. Each occurrence is recorded only under the phrase, using the actual attested wording, including proclitics and solid or spaced spelling. These occurrence bullets satisfy token coverage (§11.4) for all tokens in their spans. Do not duplicate them in component form entries or component totals; forms used independently retain their own entries and independent occurrence counts. The phrase headword identifies the entry and need not occur verbatim in the source: only its occurrence bullets are matched to the cited text. A source token cannot be claimed by overlapping phrase or form occurrences. **The test is whether a component is rendered differently inside the phrase than it would be on its own**, to carry the phrase's meaning. Where every component keeps its own rendering, the phrase is translated directly and takes no entry. Frequency, formulaic use, and solid writing are not evidence; those show univerbation (§10.12). `ʿālam ʿālmīn` qualifies — *ʿālam* is "Eternity" alone but "Age" inside it. `m_n ʿālam waʿdamā lʿālam` does not, nor does `b-ḵl ʿedān`. A phrase headword contains a space, which the entry parser must allow.
...
11. **Complete decision indexing.** Every indexed translation decision is retained as its own occurrence bullet. Each rendering count equals the number of bullets assigned to that rendering, and the headword decision total equals the sum of the rendering counts. Counts are indexed translation decisions, **not raw token occurrences**. Repetitions, same-locus witness duplicates, and common liturgical units excluded by §§10.13 and 10.18 do not increment them.
...
2. NFC normalization
3. Per-entry bullets == Σrendering counts, and decision total == Σrendering counts
4. Every source token has an attested form, except proclitics and *lā* (§10.2), untranslated acclamations (§10.16), repetitions and same-locus witness duplicates already cited (§10.13), and members of a common liturgical unit already entered (§10.18). Phrase-section occurrence bullets cover their attested token spans without duplicate component bullets (§10.6). Parenthesized editorial and rubrical labels are apparatus, not source tokens.
...
8. Every recorded rendering is traceable in that entry's own context strings, with `...` (§10.19) and the §10.15 binding resolved first; source-apparatus square-bracket characters are ignored for this traceability check only. Phrase renderings are checked against their own occurrence contexts
